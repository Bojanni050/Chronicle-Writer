...
export async function POST(
  ...
  const config = getModelConfig('continuity_check');
  ...
  const { system, user } = buildContinuityPrompt(pack.scene!, context, userInstruction);
  ...
  const parsedOutput = parseContinuityOutput(generationResult.text);
  ...
  try {
    [data] = await db
      ...
      .values({
        ...
        model: config.model,
        prompt_version: getPromptVersion('continuity_check'),
        prompt_text: `SYSTEM:\n${system}\n\nUSER:\n${user}`,
