...
export async function POST(
  ...
  const config = getModelConfig('brainstorm');
  ...
  const { system, user } = buildBrainstormPrompt(pack.scene!, context, userInstruction);
  ...
  const parsedOutput = parseBrainstormOutput(generationResult.text);
  ...
  try {
    [data] = await db
      ...
      .values({
        ...
        model: config.model,
        prompt_version: getPromptVersion('brainstorm'),
        prompt_text: `SYSTEM:\n${system}\n\nUSER:\n${user}`,
