...
export async function POST(
  ...
  const config = getModelConfig('draft');
  ...
  const { system, user } = buildDraftPrompt(pack.scene!, context, userInstruction);
  ...
  try {
    [data] = await db
      ...
      .values({
        ...
        model: config.model,
        prompt_version: getPromptVersion('draft'),
        prompt_text: `SYSTEM:\n${system}\n\nUSER:\n${user}`,
  ...
  return NextResponse.json({ ...data, parsedOutput: null, costEstimate }, { status: 201 });
}
